...
class Fault_Detection(QMainWindow, fault_detection.Ui_MainWindow,
    ...
    def setSanpshotTableView2(self):

        self.snapshotTableView_2.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)  ######允许右键产生子菜单
        self.snapshotTableView_2.customContextMenuRequested.connect(self.generateMenu)  ####右键菜单

        self.model2 = QStandardItemModel(self.snapshotTableView)
        self.snapshotTableView_2.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.snapshotTableView_2.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.snapshotTableView_2.setShowGrid(False)

        # 设置数据层次结构，4行4列
        self.model2 = QStandardItemModel(7, 4)
        # 设置水平方向四个头标签文本内容
        self.model2.setHorizontalHeaderLabels(['', '时间', '类型', '编号'])

        self.snapshotTableView_2.setModel(self.model)
    ...
    def updateSnapshotsList2(self, kind):
        snapshotsList = json.loads(ssnapshot.get_image_list(kind))
        imagesList = snapshotsList["image_list"]
        imagesListLen = len(imagesList)

        # 设置数据层次结构，4行4列
        self.model2 = QStandardItemModel(imagesListLen, 4)
        # 设置水平方向四个头标签文本内容
        self.model2.setHorizontalHeaderLabels(['', '编号', '视频时间', '时间'])
        for row in range(imagesListLen):
            item_checked = QStandardItem()
            item_checked.setCheckState(QtCore.Qt.Unchecked)
            item_checked.setCheckable(True)
            self.model2.setItem(row, 0, item_checked)
            itemIndex = QStandardItem(str(imagesList[row]["index"]))
            itemVideoTime = QStandardItem(str(imagesList[row]["video_time"]))
            itemImageTime = QStandardItem(str(imagesList[row]["image_time"]))
            self.model2.setItem(row, 1, itemIndex)
            self.model2.setItem(row, 2, itemVideoTime)
            self.model2.setItem(row, 3, itemImageTime)
        self.snapshotTableView_2.setModel(self.model)
    ...
    def generateMenu2(self, pos):
        ...
        """
        row_num = -1
        for i in self.snapshotTableView_2.selectionModel().selection().indexes():
            row_num = i.row()

        if True:  # 表格生效的行数，501行点击右键，不会弹出菜单
            menu = QMenu()  # 实例化菜单
            item1 = menu.addAction(u"详情")
            action = menu.exec_(self.snapshotTableView_2.mapToGlobal(pos))
        else:
            return

        if action == item1:
            row = self.snapshotTableView_2.currentIndex().row()
            id = int(self.model2.item(row, 1).text())
            print(row)
            print(self.model2.item(row, 1).text())
    ...
    def printline2(self, item):
        rows = []
        for index in self.snapshotTableView_2.selectionModel().selection().indexes():
            rows.append(index.row())
        rows = list(set(rows))
        for index in range(self.model2.rowCount()):
            if index in rows:
                ...
                item_checked.setCheckState(QtCore.Qt.Checked)
                item_checked.setCheckable(True)
                self.model2.setItem(index, 0, item_checked)
            else:
                item_checked = QStandardItem()
                item_checked.setCheckState(QtCore.Qt.Unchecked)
                item_checked.setCheckable(True)
