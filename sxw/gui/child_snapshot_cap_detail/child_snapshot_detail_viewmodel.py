import json
import re
import sxw.gui.child_snapshot_cap_detail.child_snapshot_detail  as child_snapshot_detail # 刚刚转为py文件的UI文件名
import cv2
...
class ChildSnapshotDetails(QMainWindow, child_snapshot_detail.Ui_MainWindow):
    ...
    def updataSnapshotInfo(self,kind,id):
        ...
        length = str(info["info"]["size_w"]) + info["info"]["unit"]
        width = str(info["info"]["size_h"]) + info["info"]["unit"]
