...
def open_new_video(video_url_path):  # 打开新视频时，调用该函数为视频创建视频内容保存路径
    ...
    # video_path = "/Users/shichunjing/Desktop/test_.mp4"  # 测试用
    # video_path = "/Users/shichunjing/Desktop/C++Primer.pdf"  # 测试用
    # video_path = "/Users/shichunjing/Desktop/动手学深度学习.pdf"
    return_dict = {
        'code': -1,  # 状态码，-1表示无意义
        'message': "null",  # 状态码信息
    ...
    if history_video_dict["devices_list"].__contains__(video_name[:-4]):  # 新视频创建失败，已经存在相同名字的历史视频
        ...
        if video_name[-3:] != "mp4":
            ...
            shutil.copyfile(video_path, device_video_path + "/" + video_name[:-4] + "/" + video_name[:-4]+".mp4")  # 复制视频副本
